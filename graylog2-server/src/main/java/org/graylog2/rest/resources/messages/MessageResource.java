...
public class MessageResource extends RestResource {
    ...
    public ResultMessage parse(@ApiParam(name = "JSON body", required = true) MessageParseRequest request) {
        ...
        final RawMessage rawMessage = new RawMessage(0, Generators.timeBasedGenerator().generate(), Tools.nowUTC(), remoteAddress, request.message().getBytes(StandardCharsets.UTF_8));
